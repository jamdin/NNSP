...
for k=2:hidden_layer_size+1
    for j=1:num_labels
    reg=reg+Theta3(j,k).^2;
    end
end
J=Jsr+(lambda/(2*m))*reg;

delta4=zeros(m,K);
delta3=zeros(m,hidden_layer_size);
...
for t=1:m
    ...
    delta4(t,:)=(a_4-yy(t,:));
    z33=[1 z3];
    ad3=(delta4(t,:)*Theta3).*sigmoidGradient(z33);
    delta3(t,:)=ad3(2:end);
    z22=[1 z2];
...
tt3=[zeros(size(Theta3,1),1) Theta3(:,2:end)];
Theta1_grad=Theta1_grad/m+(lambda/m)*tt1;
Theta2_grad=Theta2_grad/m+(lambda/m)*tt2;
Theta3_grad=Theta3_grad/m+(lambda/m)*tt3;
